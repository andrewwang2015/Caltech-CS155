...
def main():
    ...
    while (True):
        ...
        count += 1
        #print (alpha)
        coefficients.append(runLassoRegression(alpha, inputs, outputs))
        alphas.append(alpha)
        alpha += 0.001  #0.01 for lasso, 30 for ridge
    ...
    x = alphas
    fig = plt.figure()
    plt.title(r'Model weights vs. $\alpha$ (Lasso)', fontsize = 22)
    ...
    plt.legend(('w$_1$','w$_2$', 'w$_3$', 'w$_4$', 'w$_5$', 'w$_6$', 'w$_7$', 'w$_8$', 'w$_9$'), loc = 'upper right', fontsize = 14)
